...
impl PixivClient {
    ...
    pub async fn auth(&mut self, refresh_token: &str) -> reqwest::Result<PixivClientData> {
        let datetime = Utc::now().to_rfc3339_opts(SecondsFormat::Secs, false);
        let hash = md5::compute(format!("{}{}", &datetime, HASH_SECRET).as_bytes());
    ...
    async fn check_auth(&mut self) {
        ...
        let refresh = self.client_data.clone().unwrap().refresh_token;
    ...
    pub async fn get_illust_detail(&mut self, illust_id: i64) -> reqwest::Result<PixivIllustDetail> {
